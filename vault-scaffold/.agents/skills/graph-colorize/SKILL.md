...
1. Glob `$VAULT_PATH/**/*.md` excluding `_archives/`, `archive/`, `_raw/`, `.obsidian/`, `.agents/`, `.claude/`, `node_modules/`, `index.md`, `log.md`, `_insights.md`.
...
Use the vault top-level folders in this fixed order so colors are stable across runs:

| Folder | Color index | Notes |
|---|---|---|
| `concepts` | 0 (blue) | abstract patterns and mental models |
| `entities` | 1 (orange) | tools, libraries, products |
| `skills` | 2 (red) | reusable how-to knowledge |
| `references` | 3 (teal) | external reference material |
| `synthesis` | 4 (green) | cross-cutting insights |
| `projects` | 5 (yellow) | active time-bound work |
| `journal` | 6 (purple) | time-stamped log entries |
| `areas` | 4 (green / emerald) | ongoing responsibilities — living content, no end date; reuse green to signal active/alive |
| `mocs` | 5 (amber / yellow) | navigation hubs only — visually distinct from content nodes to indicate structural rather than knowledge nodes |
| `archive` | 9 (gray / muted) | retired content — visually recedes in graph to reduce noise from inactive pages |

**Notes on new zones:**
- `areas` shares green with `synthesis` to indicate living, ongoing content. If visual distinction is needed, use `#34D399` (emerald-400, packed int `3462041`) instead.
- `mocs` uses amber/yellow to mark structural nav hubs; they link outward but hold no knowledge themselves.
- `archive` always maps to gray (palette index 9, `#BAB0AC`). These nodes should recede — do not assign a vivid color here.
