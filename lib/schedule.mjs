...
// systemd reads user units from $XDG_CONFIG_HOME/systemd/user, falling back to
// ~/.config. Writing to ~/.config unconditionally puts the file where systemd is not
// looking on any system that sets XDG_CONFIG_HOME.
const SYSTEMD_DIR = path.join(
...
`;
  ...
  try {
    ...
    // systemctl can be present and still refuse: no session bus, a sandboxed or
    // relocated HOME, a container. Leaving orphaned unit files and no schedule is
    // the worst outcome, so clean up and let the caller try the next backend.
    for (const f of [svcPath, timerPath]) {
      try { fs.rmSync(f, { force: true }); } catch { /* best effort */ }
    }
    const detail = String(err.stderr || err.message || '').trim().split('\n')[0];
    throw new Error(`systemd refused the unit (${detail})`);
  }
...
export function installSchedule(time = '19:00', { dryRun = false } = {}) {
  ...
  // Try each backend in preference order. A backend can be detected and still fail
  // at registration, and on Linux that is common enough that falling through to cron
  // matters more than reporting the first failure.
  const tried = [];
  for (const kind of kinds) {
    try {
      switch (kind) {
        case 'launchd': return { ...installLaunchd(at, dryRun), tried };
        case 'systemd': return { ...installSystemd(at, dryRun), tried };
        case 'cron': return { ...installCron(at, dryRun), tried };
        case 'schtasks': return { ...installSchtasks(at, dryRun), tried };
        default: continue;
      }
    } catch (err) {
      tried.push(`${kind}: ${err.message}`);
    }
  }
  return { kind: null, action: 'all-backends-failed', target: null, tried };
}
